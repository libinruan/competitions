...
all_df.Fare = all_df.Fare.fillna(all_df.Fare.mean()) # NOTE If use median() instead, got lower CV: 0.78276
...
#%%
# # NOTE Categorical interactions 
# # CV 0.78276
# comb = list(itertools.combinations(['Name', 'Ticket'], 2))
# comb_cols = []
# for c1, c2 in comb:
#     all_df.loc[:, c1 + '_' + c2] = all_df[c1].astype(str) + '_' + all_df[c2].astype(str)
#     comb_cols.append(c1 + '_' + c2)
# label_cols = label_cols + comb_cols

#%%
# # NOTE Polynomial interaction features
# pf = PolynomialFeatures(degree = 2, interaction_only = False, include_bias = False)
# feat_poly = pf.fit_transform(all_df.loc[:, ['Age', 'Fare']])
# n = feat_poly.shape[1]
# poly_cols = [f'f_{i}' for i in range(1, n+1)]
# df_transformed = pd.DataFrame(feat_poly, columns = poly_cols)
# # all_df = pd.concat([all_df, df_transformed], axis = 1)
# all_df = pd.concat([all_df, df_transformed.reindex(all_df.index)], axis=1)
# numerical_cols = [col for col in numerical_cols + poly_cols if not col in ['Age', 'Fare']]
