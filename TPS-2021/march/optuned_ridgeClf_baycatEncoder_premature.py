...
from BayCatEncoder import BayCatEncoder
...
# numeric_features = select_numeric_features(train) # Note: scaleing for linear models with regularization. Without regularization, linear models doesn't need to be scaled simply for prediction.
...
#%%  
def bay_cat_encoding():
    connected_cat = ['cat16.cat15', 'cat16.cat0', 'cat18.cat15', 'cat18.cat16', 'cat16.cat14'] + ['cat16.cat1', 'cat4', 'cat11', 'cat2', 'cat17'] + ['cat6', 'cat10', 'cat7']
    bay_cat_encoder = BayCatEncoder(
        connected_cat, 
        'target', 
        CV=True, 
        n_fold=3, 
        ...
        drop_original=False)
    bc_encoder = bay_cat_encoder.fit(train.drop('target',axis=1), train['target'])
    train_encoded = bc_encoder.transform(train)
    test_encoded = bc_encoder.transform(test)
...
test_encoded = pd.read_pickle('test_encoded.pkl')
...
study.optimize(ridgeCLF_objective, n_trials=2)
